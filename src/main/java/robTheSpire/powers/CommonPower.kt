...
import robTheSpire.DefaultMod.Companion.makePowerPath
import robTheSpire.util.TextureLoader

//Gain 1 dex for the turn for each card played.
class CommonPower(owner: AbstractCreature?, source: AbstractCreature, amount: Int) : AbstractPower(), CloneablePowerInterface {
    var source: AbstractCreature

    // On use card, apply (amount) of Dexterity. (Go to the actual power card for the amount.)
    override fun onUseCard(card: AbstractCard, action: UseCardAction) {
        AbstractDungeon.actionManager.addToBottom(ApplyPowerAction(owner, owner,
    ...
    companion object {
        val POWER_ID = makeID("CommonPower")
