...
class PlaceholderPotion : AbstractPotion(NAME, POTION_ID, PotionRarity.COMMON, PotionSize.M, PotionColor.SMOKE) {
    ...
    override fun use(target: AbstractCreature) {
        var target: AbstractCreature? = target
        ...
        if (AbstractDungeon.getCurrRoom().phase == AbstractRoom.RoomPhase.COMBAT) {
            AbstractDungeon.actionManager.addToBottom(ApplyPowerAction(target, AbstractDungeon.player, StrengthPower(target, potency), potency))
            AbstractDungeon.actionManager.addToBottom(ApplyPowerAction(target, AbstractDungeon.player, LoseStrengthPower(target, potency), potency))
        }
    }
    ...
    companion object {
        val POTION_ID = makeID("PlaceholderPotion")
