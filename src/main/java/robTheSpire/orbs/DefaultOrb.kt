...
import robTheSpire.DefaultMod.Companion.makeOrbPath
import robTheSpire.util.TextureLoader.getTexture

class DefaultOrb : AbstractOrb() {
    ...
    override fun onEvoke() { // 1.On Orb Evoke
        AbstractDungeon.actionManager.addToBottom( // 2.Damage all enemies
                DamageAllEnemiesAction(AbstractDungeon.player, DamageInfo.createDamageMatrix(evokeAmount, true, true), DamageInfo.DamageType.THORNS, AbstractGameAction.AttackEffect.NONE))
    ...
    override fun updateAnimation() { // You can totally leave this as is.
        ...
        if (vfxTimer < 0.0f) {
            AbstractDungeon.effectList.add(DarkOrbPassiveEffect(cX, cY)) // This is the purple-sparkles in the orb. You can change this to whatever fits your orb.
    ...
    companion object {
        ...
        val ORB_ID = makeID("DefaultOrb")
        private val orbString = CardCrawlGame.languagePack.getOrbString(ORB_ID)
        val DESC = orbString.DESCRIPTION
        private val IMG = getTexture(makeOrbPath("default_orb.png"))
