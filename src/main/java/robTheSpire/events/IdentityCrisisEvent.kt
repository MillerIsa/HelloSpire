...
import robTheSpire.DefaultMod.Companion.makeEventPath
...
class IdentityCrisisEvent : AbstractImageEvent(NAME, DESCRIPTIONS[0], IMG) {
        ...
        when (screenNum) {
            0 -> when (i) {
                0 -> {
                    ...
                    val relicToAdd = RelicLibrary.starterList[AbstractDungeon.relicRng.random(RelicLibrary.starterList.size - 1)].makeCopy()
                    // Get a random starting relic
                    AbstractDungeon.getCurrRoom().spawnRelicAndObtain((Settings.WIDTH / 2).toFloat(), (Settings.HEIGHT / 2).toFloat(), relicToAdd)
                }
                1 -> {
                    ...
                    if (CardGroup.getGroupWithoutBottledCards(AbstractDungeon.player.masterDeck.purgeableCards).size() > 0) {
                ...
                2 -> {
                    ...
                    AbstractDungeon.effectList.add(ShowCardAndObtainEffect(c, (Settings.WIDTH / 2).toFloat(), (Settings.HEIGHT / 2).toFloat()))
                ...
                3 -> {
                    imageEventText.loadImage("robTheSpireResources/images/events/IdentityCrisisEvent2.png") // Change the shown image
    ...
    override fun update() { // We need the update() when we use grid screens (such as, in this case, the screen for selecting a card to remove)
        ...
        if (!AbstractDungeon.gridSelectScreen.selectedCards.isEmpty()) { // Once the grid screen isn't empty (we selected a card for removal)
            ...
            AbstractDungeon.topLevelEffects.add(PurgeCardEffect(c, (Settings.WIDTH / 2).toFloat(), (Settings.HEIGHT / 2).toFloat())) // Create the card removal effect
    ...
    companion object {
        val ID = makeID("IdentityCrisisEvent")
        ...
        val IMG = makeEventPath("IdentityCrisisEvent.png")
    }

    private var healthdamage = if (AbstractDungeon.ascensionLevel >= 15) { // If the player is ascension 15 or above, lose 5% max hp. Else, lose just 3%.
...
}//The actual number of how much Max HP we're going to lose. = 0
    init {
        screenNum = 0 // The initial screen we will see when encountering the event - screen 0;
