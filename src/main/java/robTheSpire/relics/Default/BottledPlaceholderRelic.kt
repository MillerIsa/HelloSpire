...
import robTheSpire.patches.relics.BottledPlaceholderField
import robTheSpire.util.TextureLoader.getTexture
import java.util.function.Predicate

class BottledPlaceholderRelic : CustomRelic(ID, IMG, OUTLINE, RelicTier.COMMON, LandingSound.CLINK), CustomBottleRelic, CustomSavable<Int?> {
    ...
    override fun onEquip() { // 1. When we acquire the relic
        ...
        AbstractDungeon.gridSelectScreen.open(group, 1, DESCRIPTIONS[3].toString() + name + DESCRIPTIONS[2], false, false, false, false)
    ...
    override fun onUnequip() { // 1. On unequip
        if (card != null) { // If the bottled card exists (prevents the game from crashing if we removed the bottled card from our deck for example.)
            ...
            if (cardInDeck != null) {
                BottledPlaceholderField.inBottledPlaceholderField[cardInDeck] = false // In our SpireField - set the card to no longer be bottled. (Unbottle it)
            }
        }
    }
    ...
    override fun update() {
        ...
        if (!cardSelected && !AbstractDungeon.gridSelectScreen.selectedCards.isEmpty()) {
            ...
            BottledPlaceholderField.inBottledPlaceholderField[card] = true // Use our custom spire field to set that card to be bottled.
    ...
    override fun onUseCard(targetCard: AbstractCard, useCardAction: UseCardAction) { // Whenever we use any card
        var fullHandDialog = false // Create a boolean (to prevent multiple "My hand is full!" dialogues if we have multiple cards bottled)
        ...
        while (it.hasNext()) {
            ...
            val card = it.next() // create a new AbstractCard named "card" which is equal to the current card in the for each loop
     ...
     */
        ...
        // ID, images, text.
        val ID = makeID("BottledPlaceholderRelic")
        private val IMG = getTexture(makeRelicPath("BottledPlaceholder.png"))
        private val OUTLINE = getTexture(makeRelicOutlinePath("BottledPlaceholder.png"))
    }
