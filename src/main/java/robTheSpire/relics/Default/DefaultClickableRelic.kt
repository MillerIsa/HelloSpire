...
class DefaultClickableRelic : CustomRelic(ID, IMG, OUTLINE, RelicTier.COMMON, LandingSound.CLINK), ClickableRelic {
    private var usedThisTurn = false // You can also have a relic be only usable once per combat. Check out Hubris for more examples, including other StSlib things.
    override fun onRightClick() { // On right click
        ...
        if (AbstractDungeon.getCurrRoom() != null && AbstractDungeon.getCurrRoom().phase == AbstractRoom.RoomPhase.COMBAT) { // Only if you're in combat
            ...
            AbstractDungeon.actionManager.addToBottom(TalkAction(true, DESCRIPTIONS[1], 4.0f, 2.0f)) // Player speech bubble saying "YOU ARE MINE!" (See relic strings)
            AbstractDungeon.actionManager.addToBottom(SFXAction("MONSTER_COLLECTOR_DEBUFF")) // Sound Effect Action of The Collector Nails
            AbstractDungeon.actionManager.addToBottom(VFXAction( // Visual Effect Action of the nails applies on a random monster's position.
                    CollectorCurseEffect(AbstractDungeon.getRandomMonster().hb.cX, AbstractDungeon.getRandomMonster().hb.cY), 2.0f))
     ...
     */
        // ID, images, text.
        val ID = makeID("DefaultClickableRelic")
        private val IMG = getTexture(makeRelicPath("default_clickable_relic.png"))
        private val OUTLINE = getTexture(makeRelicOutlinePath("default_clickable_relic.png"))
    }
